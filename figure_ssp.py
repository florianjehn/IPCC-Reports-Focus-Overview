...
import count_ssp_ipcc

def plot_all_ssp_by_ar(ipcc_counts, meta, cmap):
    ...
    ssps = list(count_ssp_ipcc.create_ssp_dict().keys())
    # Plot the seperate temps
    ax = counts_meta.groupby("AR")[ssps].mean().plot(kind="bar", stacked=True, cmap=cmap)    
    ...
    fig.tight_layout()
    plt.savefig("Figures"+ os.sep +"AR_all_ssp_and_grouped.png", dpi=200)
    plt.close()
...
if __name__ == "__main__":
    ...
    # get the data    
    ipcc_counts = rp_da.read_ipcc_counts_ssp()
    ...
    # Plot    
    plot_all_ssp_by_ar(ipcc_counts, meta, cmap)